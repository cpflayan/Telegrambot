...
def add(update: Update, context: CallbackContext) -> int:
        ...
        cursor.execute('INSERT INTO transactions (chat_id, action, amount, date, time) VALUES (?, ?, ?, ?, ?)', (chat_id, "add", amount, currentDate, currentTime))
        conn.commit()
...
def subtract(update: Update, context: CallbackContext) -> int:
        ...
        cursor.execute('INSERT INTO transactions (chat_id, action, amount, date, time) VALUES (?, ?, ?, ?, ?)', (chat_id, "subtract", amount, currentDate, currentTime))
        conn.commit()
...
# 執行 INSERT 操作，插入新紀錄
        cursor.execute('INSERT INTO transactions (chat_id, action, amount, date, time) VALUES (?, ?, ?, ?, ?)', (chat_id, "fee", amount, currentDate, currentTime))
        conn.commit()
...
def calculate_balance(update: Update, context: CallbackContext) -> int:
    ...
    if date_str.endswith("record"):
        ...
        cursor.execute('INSERT INTO transactions (chat_id, action, amount, date, time) VALUES (?, ?, ?, ?, ?)', (chat_id, "count", balance, currentDate, currentTime))
        conn.commit()
...
def list_records(update: Update, context: CallbackContext) -> int:
    ...
    # 查询指定日期的 (ID) 时间 金额 数据
    cursor.execute('SELECT id, time, amount, action FROM transactions WHERE (action="add" OR action="subtract") AND date = ? AND chat_id = ?', (date_to_list, chat_id))
    ...
    if not records:
        ...
        for record in records:
            ...
            response_text += f"({record[0]}) {record[1]} {action_symbol}{record[2]} 元\n"
